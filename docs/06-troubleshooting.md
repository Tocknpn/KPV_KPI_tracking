...
| Roster screen shows an empty table for a month | Nobody has uploaded/edited the roster for that exact month yet | The Roster screen no longer carries forward an earlier month's data for its own display — go to that month and upload/edit the roster, even if nothing actually changed from the month before. (KPI Report and other calculations still fall back to the nearest earlier month automatically — only the Roster screen's own view requires an exact-month upload.) |
