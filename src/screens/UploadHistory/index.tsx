...
export default function UploadHistory() {
  ...
  useEffect(() => { load() }, [token, selectedYear, selectedMonth, filterType, filterBranch])
  ...
  return (
    <AppShell title="Upload History">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          <h2 className="font-headline-lg text-headline-lg text-on-surface">Upload History & Coverage</h2>
          <p className="text-on-surface-variant text-body-md mt-1">
            Track which branches have submitted data for {MONTHS[selectedMonth - 1]} {selectedYear}
          </p>
        </div>
      ...
      {/* Alert banner — branches not uploaded */}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant/10">
              {loading ? (
                ...
                const status   = !c.target_uploaded ? 'error'
                  : c.days_with_entries === 0 ? 'warning'
                  : entryPct >= 80 ? 'success' : 'neutral'
                const statusLabel = !c.target_uploaded ? 'No Target'
                  : c.days_with_entries === 0 ? 'No Entries'
                  : entryPct >= 80 ? 'Good' : 'Partial'
                ...
                return (
                  <tr key={c.branch_id} className={`transition-colors hover:bg-surface-container/20 ${status === 'error' ? 'bg-error-container/5' : status === 'warning' ? 'bg-secondary-container/5' : ''}`}>
                    ...
                    <td className="px-6 py-4 min-w-[180px]">
                      {progressBar(c.days_with_entries, c.days_in_month, 'bg-primary')}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <StatusBadge label={statusLabel} variant={status as 'success' | 'warning' | 'error' | 'neutral'} />
                    </td>
                  </tr>
                )
              })}
            ...
            <tbody className="divide-y divide-outline-variant/10">
              {logs.length === 0 ? (
                <tr><td colSpan={8} className="py-10 text-center text-on-surface-variant text-body-sm">
                  No upload history found for this filter.
                </td></tr>
              ) : logs.map(log => (
                <tr key={log.id} className="hover:bg-surface-container/20 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <StatusBadge
                      label={log.upload_type === 'daily' ? 'Daily' : 'Target'}
                      variant={log.upload_type === 'daily' ? 'info' : 'gold'}
                    />
                  </td>
                  <td className="px-5 py-3 text-body-sm text-on-surface-variant max-w-[180px] truncate" title={log.filename}>
                    {log.filename || '—'}
                  </td>
